				...
				@foreach ($appointments as $appointment)
					<tr class="odd:bg-white even:bg-gray-100 border-b font-medium">
						...
						<td class="px-6 py-3 flex flex-col gap-1 xl:block xl:space-x-1">
                            @if (now()->diffInMinutes($appointment->created_at) < 60)
                                <form action="{{ route('user.appointment.cancel', $appointment->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-3 py-1 rounded-lg bg-red-500 hover:bg-red-600 text-white">Cancel</button>
                                </form>
                            @else
                                <button type="submit" class="px-3 py-1 rounded-lg bg-gray-300 text-white" disabled>Cancel</button>
                            @endif
						</td>
					</tr>
				@endforeach
